(function () {
  ...
  async function procesarArchivosPdf(archivos, origen) {
    ...
    for (const archivo of archivos) {
      ...
      ui.pText.textContent = `Cargando mapeo de referencia…`;
      try {
        // ── ÚNICO MÉTODO: comparar imágenes nuevas contra imágenes de referencia del mapeo ──
        let bloquesFinales = null;

        // Buscar referencia guardada en IndexedDB
        ...
        for (const nombre of nombresPatrones) {
          ...
          if (ref?.imagenes?.length && ref?.bloques?.length) {
            ...
            console.log(`[MAU][TRABAJAR] Referencia encontrada: "${nombre}" (${ref.imagenes.length} imgs, ${ref.bloques.length} bloques)`);
            break;
          }
        }

        if (!referenciaEncontrada) {
          mostrarToast(`No hay mapeo guardado. Hacé un mapeo primero desde "Aprender".`);
          console.warn("[MAU][TRABAJAR] Sin referencia en IndexedDB. Hacé un mapeo primero.");
          continue;
        }

        // Renderizar páginas del PDF nuevo como imágenes
        ...
        // Claude machea las páginas nuevas contra el mapeo guardado — 1 sola llamada
        ui.pText.textContent = `Macheando con mapeo guardado…`;
        console.log(`[MAU][TRABAJAR] Comparando ${nuevasPaginas.length} páginas contra ${referenciaEncontrada.bloques.length} bloques…`);
        bloquesFinales = await window.MAUStorage.compararConReferencia(nuevasPaginas, referenciaEncontrada);

        if (!bloquesFinales || !bloquesFinales.length) {
          mostrarToast(`«${archivo.name}» no coincide con el mapeo guardado. Revisá que sea el archivo correcto.`);
          console.warn("[MAU][TRABAJAR] Sin resultado del macheo. El archivo puede ser muy distinto al mapeo.");
          continue;
        }

        console.log(`[MAU][TRABAJAR] ✅ Macheo exitoso: ${bloquesFinales.length} bloque(s)`);
